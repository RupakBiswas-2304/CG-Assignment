...
class Mesh:
    ...
    def rotateY(self, angle):
        self.vertices = list(np.dot(np.array(self.vertices), np.array([[math.cos(
            angle), 0, -math.sin(angle)], [0, 1, 0], [math.sin(angle), 0, math.cos(angle)]])))

    def rotateX(self, angle):
        self.vertices = list(np.dot(np.array(self.vertices), np.array([[1, 0, 0], [
                             0, math.cos(angle), -math.sin(angle)], [0, math.sin(angle), math.cos(angle)]])))

    def rotateZ(self, angle):
        self.vertices = list(np.dot(np.array(self.vertices), np.array([[math.cos(
            angle), -math.sin(angle), 0], [math.sin(angle), math.cos(angle), 0], [0, 0, 1]])))

    def rotate(self, angle: tuple):
        self.vertices = self.rotateZ(self.rotateY(
            self.rotateX(self.vertices, angle[0]), angle[1]), angle[2])
